...
export function CompanyWalletPage() {
  ...
  useEffect(() => {
    ...
    if (paymentStatus === 'success') {
      toast.success(t('wallet.paymentResult.success'));
      queryClient.invalidateQueries({ queryKey: ['wallet'] });
      queryClient.invalidateQueries({ queryKey: ['payments'] });
    } else if (paymentStatus === 'error') {
      toast.error(t('wallet.paymentResult.error'));
    } else if (paymentStatus === 'cancel') {
      toast(t('wallet.paymentResult.cancel'), { icon: 'ℹ️' });
    }
  ...
  const sePayMutation = useMutation({
    mutationFn: (amount: number) => {
      ...
      const returnTo = encodeURIComponent('/company/wallet');
      return paymentsApi.createSePayCheckout({
        amount,
        purpose: 'TopUp',
        successUrl: `${origin}/payment/success?returnTo=${returnTo}`,
        errorUrl: `${origin}/payment/error?returnTo=${returnTo}`,
        cancelUrl: `${origin}/payment/cancel?returnTo=${returnTo}`,
      });
    },
    onSuccess: (data) => {
