...
export function ApplicationStatusActionsCard({
    ...
    return (
        <Card className="p-6">
            <h2 className="text-lg font-semibold text-gray-900">{t('applicationDetail.reviewActionsTitle')}</h2>
            {canReviewStatus ? (
                <div className="mt-5 space-y-4">
                    <div className="flex flex-wrap gap-3">
                        <Button onClick={onAccept} isLoading={isAccepting}>
                            {t('applicationDetail.btnAcceptApplication')}
                        </Button>
                    </div>

                    <div>
                        <label className="mb-2 block text-sm font-medium text-gray-700">
                            {t('applicationDetail.rejectReasonInputLabel')}
                        </label>
                        <textarea
                            rows={4}
                            value={rejectReason}
                            onChange={(event) => onRejectReasonChange(event.target.value)}
                            placeholder={t('applicationDetail.rejectReasonInputPlaceholder')}
                        ...
                        <div className="mt-3">
                            <Button variant="outline" isLoading={isRejecting} onClick={onReject}>
                                {t('applicationDetail.btnRejectApplication')}
                            </Button>
                        </div>
                    </div>
                </div>
            ) : (
                <p className="mt-4 text-sm text-gray-600">{t('applicationDetail.reviewClosedHint')}</p>
            )}
        </Card>
    );
}
